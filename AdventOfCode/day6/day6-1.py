...
def decrementLanternFishes():
    for lanternfish in lanternfishes:
        value = lanternfish.value
        lanternfish.value = value - 1 if value > 0 else 6
...
for line in f:
    ...
    for i in parts:
        lanternfishes.append(lanternfish(int(i)))

else:
    totalZeros = 0
    for i in range(80):
        decrementLanternFishes()
        for _ in range(totalZeros):
            lanternfishes.append(lanternfish(8))

        totalZeros = getTotalZeros()
